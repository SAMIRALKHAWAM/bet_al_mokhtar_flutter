    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 200) {
        ...
        if (cubit.hasMore && !cubit.isLoadingMore) {
          cubit.Table_get(isLoadMore: true);
        }
      }
    });
  }
      ...
      builder: (context, state) {
        ...
        return Scaffold(
          ...
          body: Padding(
            ...
            child: Builder(
              builder: (context) {
                if (state is LoadingState && tables.isEmpty) {
                  return Center(child: CircularProgressIndicator());
                } else if (state is tableErrorState && tables.isEmpty) {
                  return Center(child: Text('خطأ في تحميل الطاولات'));
                ...
                return GridView.builder(
                  ...
                  itemCount: tables.length + (cubit.hasMore ? 1 : 0),
                  ...
                  itemBuilder: (context, index) {
                    if (index == tables.length) {
                      // عنصر تحميل البيانات في نهاية القائمة
                        ...
                        if (isAvailable) {
                          showDialog(
                            ...
                            builder: (_) => AlertDialog(
                              ...
                              content: Text('هل تريد فتح الطاولة رقم ${table.tableNumber}?'),
                              actions: [
                                TextButton(
                                ...
                                TextButton(
                                  onPressed: () {
                                    cubit.table_change_statu(table_id: table.id);
                                    cubit.Table_get();

                                    Navigator.push(
                                      context,
                                      MaterialPageRoute(
                                        builder: (_) => WaiterOrderInterface(
                                          table.id,
                                          table.tableNumber,
                                          table.invoice_id,
                                        ),
